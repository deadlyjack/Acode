...
function sortDir(
  ...
  list.map((item) => {
    ...
    if (typeof item.isDirectory !== 'boolean') {
      if (isDir(item.type)) item.isDirectory = true;
    }
...
function _convertToFile(url) {
  ...
  const providerRegex =
    /content\:\/\/(((?![:<>"\/\\\|\?\*]).)*)(\/external_storage\/|\/(file\/)?storage\/emulated\/0\/|\/external_dir\/)/;
...
function idGenereator() {
  return parseInt(
    (((1 + Math.random()) * 0x10000) | 0).toString(10).substring(1)
  );
}
 ...
 */
function decodeText(arrayBuffer, encoding = 'utf-8') {
