...
public class IntentPlugin extends CordovaPlugin {
    ...
    private JSONObject getIntentJson(Intent intent) {
        ...
        try {
            ...
            intentJSON.put("extras", toJsonObject(extras));
            intentJSON.put("action", intent.getAction());
            intentJSON.put("categories", intent.getCategories());
            intentJSON.put("flags", intent.getFlags());
            intentJSON.put("component", intent.getComponent().getPackageName());
            intentJSON.put("package", intent.getPackage());
            intentJSON.put("data", uri);

            if (uri != null && !intent.getAction().equals(intent.ACTION_SEND)) {
                if ("content".equals(uri.getScheme())) {
                    String filePath = null;
                    if (intent.hasExtra("local.uri")) {
                        filePath = intent.getExtras().getString("local.uri");
                    } else {
                        try {
                            ...
                            if (path != null && !path.isEmpty()) {
                                ...
                                filePath = "";
                                intentJSON.put("error", "Could not get file's local uri");
                            }
                            cursor.close();
                        } catch (Exception e) {
                        ...
                        try {
                            ...
                            if (index >= 0) {
                                ...
                                if (filename != null && !filename.isEmpty()) {
                                    intentJSON.put("filename", filename);
                                }
                            }
                            cursor.close();
                        } catch (Exception e) {
                            Log.e("FILENAME_CURSOR_ERROR", e.getMessage());
                        }
                    }

                    intentJSON.put("fileUri", filePath);

                } else if ("file".equals(uri.getScheme())) {

                    intentJSON.put("fileUri", uri);
                    ...
                    if (uri != null && "content".equals(uri.getScheme())) {
                        ...
                        try {
                            ...
                            if (path != null && !path.isEmpty()) {
                                ...
                                Log.d("FILEPATH_NOT_FOUND", "cannot get file path");
